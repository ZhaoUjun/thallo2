...
function venderPrefixed(styleName) {
	return styleName.replace(/^(Moz|ms)[A-Z]/, val => {
		...
		return `-${val.slice(0, index).toLocaleLowerCase()}-${val
			.slice(index)
			.toLocaleLowerCase()}`;
	});
}
...
const styleOps = {
	...
	update: (node, nextStyles, preStyles) => {
		const { style } = node;
		for (const styleName in nextStyles) {
			processStyle(style, styleName, nextStyles[styleName]);
		}
		for (const styleName in preStyles) {
